...
export function initialize_popup(state: any) {
  ...
  if (ImGui.BeginPopupModal("Enter ev3 address", null, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove)) {
    if (connecting && !error) {
      ...
      if (ImGui.Button("Connect"))
        connect(ev3_address.value.buffer, state);
      if (error)
...
function connect(ev3_address: string, state: any) {
  connecting = true;
  state.socket = new WebSocket(`ws://${ev3_address}:4567`);
